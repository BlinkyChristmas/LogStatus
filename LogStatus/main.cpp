...
auto printTableRow(std::ostream& output, const std::string& client, ConnectState state, bool audioerr, bool playerr, const std::string& playname, const std::string& ipAddress) -> void;
...
auto printTable(std::ostream& output, const ServerStatusEntry& serverstatus, ClientCollection& clients, ConnectionStatus& status, ErrorCollection& clienterrors) -> void {
    ...
    for (const auto& client : clients.clients) {
        ...
        if (status.contains(client.name)) {
            ...
            ipAddress = status[client.name].last().address;
 ...
 //       printTableRow(output, client.name, state, has_error);
        printTableRow(output, client.name, state, audio_error, play_error, playname, ipAddress);
    }
...
auto printTableRow(std::ostream& output, const std::string& client, ConnectState state, bool audioerr, bool playerr, const std::string& playname, const std::string& ipAddress) -> void {
    ...
    output << "\t\t\t\t<td style=\"background-color:" << concolor << ";\">" << ipAddress << "</td>\n";
